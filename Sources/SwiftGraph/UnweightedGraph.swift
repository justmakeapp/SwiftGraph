...
open class UnweightedGraph<V: Equatable>: Graph {
    ...
    public var edges: [[UnweightedEdge]] = .init() // adjacency lists
...
public extension Graph where E == UnweightedEdge {
    ...
    ///               Default is false.
    static func withPath(_ path: [V], directed: Bool = false) -> Self {
    ...
    ///               Default is false.
    static func withCycle(_ cycle: [V], directed: Bool = false) -> Self {
    ...
    func addEdge(fromIndex: Int, toIndex: Int, directed: Bool = false) {
    ...
    /// - parameter directed: Is the edge directed? (default `false`)
    func addEdge(from: V, to: V, directed: Bool = false) {
