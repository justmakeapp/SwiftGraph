...
public extension WeightedGraph where W: Comparable {
    ...
    func mst(start: Int = 0) -> [WeightedEdge<W>]? {
        ...
        var result = [WeightedEdge<W>]() // the final MST goes in here
        var pq = PriorityQueue<WeightedEdge<W>>(ascending: true) // minPQ
        var visited = [Bool](repeating: false, count: vertexCount) // already been to these
